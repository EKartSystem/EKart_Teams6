...
namespace EKartMVC.Controllers
{
    public class ShopController : Controller
    {
        ...
        {
            var token = HttpContext.Session.GetString("jwt");
            ViewBag.Categories = await _service.GetCategoriesAsync(token) ?? new List<CategoryDto>();
            ViewBag.Suppliers = await _service.GetSuppliersAsync(token) ?? new List<SupplierDto>();
            return View();
        }
        ...
        public async Task<IActionResult> Create(ProductDto dto)
        {
            ...
            {
                // Must re-fetch these so the dropdowns don't crash on re-render
                ViewBag.Categories = await _service.GetCategoriesAsync(token) ?? new List<CategoryDto>();
                ViewBag.Suppliers = await _service.GetSuppliersAsync(token) ?? new List<SupplierDto>();
                return View(dto);
            }
            ...
            // Re-fetch here as well for the error state
            ViewBag.Categories = await _service.GetCategoriesAsync(token) ?? new List<CategoryDto>();
            ViewBag.Suppliers = await _service.GetSuppliersAsync(token) ?? new List<SupplierDto>();
